...
def get_range_proba(predict, label, delay=7):
    # Übergänge werden in den Labels gesucht
    splits = np.where(label[1:] != label[:-1])[0] + 1
    # Es wird überprüft ob das erste Label eine Anomalie ist
    is_anomaly = label[0] == 1
    # Vorhersage wird einer neuen Variable hinzugefügt
    new_predict = np.array(predict)
    pos = 0

    #Iteration über die einzelnen Segmente 
    for sp in splits:
        if is_anomaly:
            # Wenn eine Anomalie im Segment vorliegt, überprüfe auf Anomalie-Vorhersagen innerhalb der Verzögerungszeit
            if 1 in predict[pos:min(pos + delay + 1, sp)]:
                # Setze das gesamte Segment auf Anomalie 
                new_predict[pos: sp] = 1
            else:
                # Setze das gesamte Segment auf keine Anomalie
                new_predict[pos: sp] = 0
            #Anomalie wird gewechselt
...
def reconstruct_label(timestamp, label):
    ...
    # Berechne das minimale Intervall zwischen Timestamps
    interval = np.min(np.diff(timestamp_sorted))

    # Konvertiere Labels in ein numpy-Array
    label = np.asarray(label, np.int64)
    # Sortiere die Labels basierend auf den Timestamps
    label = np.asarray(label[index])
    #Berechne die Indizes für due rekonstruierten Labels
    idx = (timestamp_sorted - timestamp_sorted[0]) // interval

    # Erstelle ein neues Labels-Array und setze die rekonstruierten Labels
...
# Bewertet die Anomalieerkennungsergebnisse
...
# Verschiebt ein Array um eine bestimmte Anzahl von Positionen
...
# Bewertet die Anomalieerkennung
def eval_anomaly_detection(model, all_train_data, all_train_labels, all_train_timestamps, all_test_data, all_test_labels, all_test_timestamps, delay):
    ...
    #Repräsentationen mit Masking
    all_train_repr = {}
    all_test_repr = {}
    #Repräsentationen without Masking
    all_train_repr_wom = {}
    all_test_repr_wom = {}
    for k in all_train_data:
        ...
        # Repräsentationen der Train/Testdaten mit Maskierung werden erzeugt
        ...
        all_test_repr[k] = full_repr[len(train_data):]
         ...
         # Repräsentationen der Train/Testdaten ohne Maskierung werden erzeugt
    ...
    for k in all_train_data:
        ...
        # Berechnung des Fehlers zwischen den kodierten Darstellungen mit und ohne Maskierung
        ...
        #Gleitender Durchschnitt der Fehler
        ...
        #Berücksichtigung des Verzögerungsschwellenwerts
